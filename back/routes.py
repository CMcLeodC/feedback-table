...
#                 Feedback.content_id == ContentsFeedbackSubcategories.content_id,
#                 Feedback.subcategory_id == ContentsFeedbackSubcategories.subcategory_id
#             ), isouter=True).where(and_(True, *filters))
...
def feedback():
    ...
    page = int(request.args.get('page', 1))
    per_page = int(request.args.get('per_page', 30))
    last_seen_created_at = int(request.args.get('previous_page', 0))
    ...
    if last_seen_created_at:
        query = query.filter(Feedback.created_at > last_seen_created_at)
